namespace Demo.examples.cs.controls
{
    using System;
    ...
    class GizmoMaterial : MeshBasicMaterial
    {
        ...
        public GizmoMaterial()
        {
            ...
            this.Side = ThreeCs.Three.FrontSide;
    ...
    class TransformGizmoTranslate : TransformGizmo
    {
    	...
    	public void setActivePlane (string axis, Vector3 eye ) 
        {
			...
			if ( axis == "X" ) {
                ...
                if (Math.Abs(eye.Y) > Math.Abs(eye.Z)) this.activePlane = (Object3D)this.planes["XZ"];
			}

			if ( axis == "Y" ){
                this.activePlane = (Object3D)this.planes["XY"];
                if (Math.Abs(eye.X) > Math.Abs(eye.Z)) this.activePlane = (Object3D)this.planes["YZ"];
			}

			if ( axis == "Z" ){
                this.activePlane = (Object3D)this.planes["XZ"];
                if (Math.Abs(eye.X) > Math.Abs(eye.Y)) this.activePlane = (Object3D)this.planes["YZ"];
			}
