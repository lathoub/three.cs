namespace Demo.examples
{
    ...
    class webgl_custom_attributes_particles3 : Example
    {
        ...
        public override void Load(Control control)
        {
            ...
            {
                { "size", new ThreeCs.Renderers.Shaders.Attribute() { { "type", "f" }, { "value", new float[0] } } },
                { "ca",   new ThreeCs.Renderers.Shaders.Attribute() { { "type", "c" }, { "value", new Color[0] } } },
            };
        ...
        public override void Render()
        {
            ...
            {
                if (i < vc1)
                    ((float[])attributes["size"]["value"])[i] = (float)Math.Max(0, 26 + 32 * Math.Sin(0.1 * i + 0.6 * time));
            }
